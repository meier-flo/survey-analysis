...
##### Calc Bootstrapped Difference 
diffMeansBoot.refinding.frustration<-calcDiffMeanCI(refinding.frustration.relation.sometimes.frust,refinding.frustration.relation.never.frust)
...
################################################
############### What re-finding strategies do people use whoe say re-finding options are good?
### we need data.frame refinding.option.relation
refinding.option.relation.numeric <- as.data.frame(lapply(refinding.option.relation,as.numeric))

refinding.options.good<-refinding.option.relation.numeric%>%filter(rf.option==1)
refinding.options.bad<-refinding.option.relation.numeric%>%filter(rf.option==2)

refinding.options.good<-refinding.options.good%>%select(-rf.option)
refinding.options.bad<-refinding.options.bad%>%select(-rf.option)

##### Calc Bootstrapped Difference 
diffMeansBoot.refinding.option<-calcDiffMeanCI(refinding.options.good,refinding.options.bad)
##### Plotting to visualise difference 
ggplot(diffMeansBoot.refinding.option,aes(x=1,y=Mean))+geom_errorbar(aes(ymin=LowerCI,ymax=UpperCI),width=.1)+geom_point(size=5)+facet_wrap(~Feature,scales="free_y")+geom_hline(aes(yintercept=0),color="red",linetype="dashed")+theme(axis.ticks = element_blank(), axis.text.x = element_blank())+xlab("")



#################
##### Make a multiplot for attachment 
a<-ggplot(diffMeansBoot.refinding.preserv.freq,aes(x=1,y=Mean))+geom_errorbar(aes(ymin=LowerCI,ymax=UpperCI),width=.1)+geom_point(size=3)+facet_wrap(~Feature,scales="free_y")+geom_hline(aes(yintercept=0),color="red",linetype="dashed")+theme(axis.ticks = element_blank(), axis.text.x = element_blank())+xlab("")+ggtitle("Preserving Frequency (never/at least rarely) and Re-finding Strategies")
b<-ggplot(diffMeansBoot.refinding.difficulty,aes(x=1,y=Mean))+geom_errorbar(aes(ymin=LowerCI,ymax=UpperCI),width=.1)+geom_point(size=3)+facet_wrap(~Feature,scales="free_y")+geom_hline(aes(yintercept=0),color="red",linetype="dashed")+theme(axis.ticks = element_blank(), axis.text.x = element_blank())+xlab("")+ggtitle("Re-finding Difficulty (easy/hard) and Re-finding Stategies")
c<-ggplot(diffMeansBoot.refinding.frustration,aes(x=1,y=Mean))+geom_errorbar(aes(ymin=LowerCI,ymax=UpperCI),width=.1)+geom_point(size=3)+facet_wrap(~Feature,scales="free_y")+geom_hline(aes(yintercept=0),color="red",linetype="dashed")+theme(axis.ticks = element_blank(), axis.text.x = element_blank())+xlab("")+ggtitle("Re-finding Frustration (never/at least rarely) and Re-finding Strategies")
d<-ggplot(diffMeansBoot.preserving.option,aes(x=1,y=Mean))+geom_errorbar(aes(ymin=LowerCI,ymax=UpperCI),width=.1)+geom_point(size=3)+facet_wrap(~Feature,scales="free_y")+geom_hline(aes(yintercept=0),color="red",linetype="dashed")+theme(axis.ticks = element_blank(), axis.text.x = element_blank())+xlab("")+ggtitle("Preserving Options Satisfaction (satisfied/not satisfied) and Preserving Strategies")
e<-ggplot(diffMeansBoot.refinding.option,aes(x=1,y=Mean))+geom_errorbar(aes(ymin=LowerCI,ymax=UpperCI),width=.1)+geom_point(size=3)+facet_wrap(~Feature,scales="free_y")+geom_hline(aes(yintercept=0),color="red",linetype="dashed")+theme(axis.ticks = element_blank(), axis.text.x = element_blank())+xlab("")+ggtitle("Re-finding Options Satisfaction (satisfied/not satisfied) and Re-finding Strategies")
