...
a<-ggplot(diffMeansBoot.refinding.preserv.freq,aes(x=1,y=Mean))+geom_errorbar(aes(ymin=LowerCI,ymax=UpperCI),width=.1)+geom_point(size=5)+facet_wrap(~Feature,scales="free_y")+geom_hline(aes(yintercept=0),color="red",linetype="dashed")+theme(axis.ticks = element_blank(), axis.text.x = element_blank())+xlab("")+ggtitle("Preserving Frequency (frequent/non-frequent) Keepers and Re-finding Strategies")
b<-ggplot(diffMeansBoot.refinding.difficulty,aes(x=1,y=Mean))+geom_errorbar(aes(ymin=LowerCI,ymax=UpperCI),width=.1)+geom_point(size=5)+facet_wrap(~Feature,scales="free_y")+geom_hline(aes(yintercept=0),color="red",linetype="dashed")+theme(axis.ticks = element_blank(), axis.text.x = element_blank())+xlab("")+ggtitle("Re-finding Rating (easy/hard) and Re-finding Stategies")
c<-ggplot(diffMeansBoot.refinding.frustration,aes(x=1,y=Mean))+geom_errorbar(aes(ymin=LowerCI,ymax=UpperCI),width=.1)+geom_point(size=5)+facet_wrap(~Feature,scales="free_y")+geom_hline(aes(yintercept=0),color="red",linetype="dashed")+theme(axis.ticks = element_blank(), axis.text.x = element_blank())+xlab("")+ggtitle("Re-finding Frustration (never/occasionaly) and Re-finding Strategies")
d<-ggplot(diffMeansBoot.preserving.option,aes(x=1,y=Mean))+geom_errorbar(aes(ymin=LowerCI,ymax=UpperCI),width=.1)+geom_point(size=5)+facet_wrap(~Feature,scales="free_y")+geom_hline(aes(yintercept=0),color="red",linetype="dashed")+theme(axis.ticks = element_blank(), axis.text.x = element_blank())+xlab("")+ggtitle("Preserving Options Rating (good/bad) and Preserve Strategies")
e<-ggplot(diffMeansBoot.refinding.option,aes(x=1,y=Mean))+geom_errorbar(aes(ymin=LowerCI,ymax=UpperCI),width=.1)+geom_point(size=5)+facet_wrap(~Feature,scales="free_y")+geom_hline(aes(yintercept=0),color="red",linetype="dashed")+theme(axis.ticks = element_blank(), axis.text.x = element_blank())+xlab("")+ggtitle("Re-finding Options Rating (good/bad) and Re-finding Strategies")
